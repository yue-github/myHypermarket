<template>
	<view>
		...
		<view class="zaiui-banner-swiper-box">
			...
			<!-- #endif -->
			<!-- #ifdef APP-PLUS -->
			<swiper class="swiper" :circular="circular" @animationfinish="bannerSwiper" :indicator-dots="indicatorDots" :autoplay="autoplay" :interval="interval" :duration="duration">
				 <swiper-item v-for="(item,index) in bannerList" :key="index">
		...
		<!--限时秒杀-->
		<!-- #ifdef APP-PLUS -->
		<view class="zaiui-limited-seckill-box" style="background-color:#ff594b">
		<!-- #endif -->
		<!-- #ifndef APP-PLUS -->
