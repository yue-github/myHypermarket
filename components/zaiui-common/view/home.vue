<template>
	<view class="zaiui-home-box" :class="show?'show':''">
		<!--欢迎-->
		<welcome-tip content="中午好，wutongyue" :show="true" :c_s="3000" @closeFinish="welcomeClose"/>
			...
			<view class="zaiui-flex-tab show">
				<view class="flex text-white">
					<view class="basis-xxl">
						<scroll-view scroll-x class="nav z" scroll-anchoring="true" :scroll-left="headTab.scrollLeft">
		...
		<view class="zaiui-view-content" :class="[viewContent.welcome?'welcome':'',headTab.TabCur==0?'show':'']">
			...
			<view class="zaiui-swiper-box">
				<swiper class="screen-swiper square-dot c" autoplay circular indicator-dots :current="swiperInfo.index" @change="swiperChange" duration="200">
			...
			<!-- #endif -->
			<!-- #ifdef APP-PLUS -->
			<view class="zaiui-swiper-box" style="margisn-top:-10upx">
				<my-swiper height="100" circular ref="QSGradientSwiper" @bannerChange="bannerChange"></my-swiper>
			</view>
			<!-- #endif -->
...
<script>
	...
	import mySwiper from '@/components/QS-Gradient-Swiper/QS-Gradient-Swiper-vue.vue';
	export default {
		name: 'home',
		components: { 
			...
			gridSortList, modalImg, mySwiper
		},
		data() {
			return {
		...
		methods: {
			...
			// 设置轮播图的项目数据
			setBannerItemData(){
				if(this.$refs.QSGradientSwiper)
					this.$refs.QSGradientSwiper.setData(
						this.swiperInfo.list.map(item=>{
							let obj = Object.assign({},item,{path:item.swiper,backgroundColor: 'transparent'});
							return obj;
						})
					);
			},
